...
-(CGSize)getSize:(NSString *)string
{
	...
	{ 
		...
		textToPrint[i] = [string characterAtIndex:i] - 31;
	}
...
{ 
	...
	{ 
		...
		textToPrint[i] = [self.text characterAtIndex:i] - 31; 
	...
	if(autoframe)
	{
		...
		switch (self.justify) {
			case kLeft:
				...
				break;
			case kCenter:
				newFrame = (CGRect){(self.fixedLoc.x) - (size.width/2), self.frame.origin.y, size.width+14, size.height+2};
				self.frame = newFrame;
				sizeDidChange = YES;
				break;
			case kRight:
